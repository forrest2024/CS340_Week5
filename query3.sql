...
SELECT actor.actor_id, actor.first_name, actor.last_name, COALESCE(SUM(film.length), 0) AS total_combined_film_length FROM actor
    JOIN film_actor ON actor.actor_id = film_actor.actor_id
        JOIN film_category ON film_actor.film_id = film_category.film_id
            JOIN category ON film_category.category_id = category.category_id
                LEFT JOIN film ON film_actor.film_id = film.film_id
                    AND category.category_id = (SELECT category.category_id FROM category WHERE category.name = 'Animation')
